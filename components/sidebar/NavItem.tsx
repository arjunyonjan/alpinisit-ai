...
export default function NavItem({ item, level = 0, pathname, onClose }: any) {
  ...
  // Fetch dynamic children if needed
  useEffect(() => {
  ...
  return (
    <div>
      <div className={`flex items-center justify-between rounded-2xl px-4 py-3 transition-all ${isActive ? "bg-blue-50 text-blue-700" : "text-gray-600 hover:bg-gray-100"} ${level > 0 ? "ml-6" : ""}`}>
        <Link href={item.href} onClick={onClose} className="flex items-center gap-3 flex-1">
      ...
      {hasChildren && isOpen && (
        <div className="ml-2 mt-1 space-y-1">
          ...
          {childrenToRender?.map((child: any) => (
            <NavItem key={child.href} item={child} level={level + 1} pathname={pathname} onClose={onClose} />
          ))}
        </div>
      )}
    </div>
