...
import { 
  ...
  Code, Table, Eye, EyeOff, Save, Sparkles, X
} from "lucide-react";
...
export default function NewNotePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch("/api/notes", {
      ...
      body: JSON.stringify({ title, tags, status, content }),
    });
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50">
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-t-2xl border border-gray-200 p-2 flex flex-wrap gap-1">
          <button onClick={() => insertMarkdown("**", "**")} className="p-2 rounded-lg hover:bg-gray-100 transition" title="Bold (Ctrl+B)"><Bold size={18} /></button>
          <button onClick={() => insertMarkdown("*", "*")} className="p-2 rounded-lg hover:bg-gray-100 transition" title="Italic (Ctrl+I)"><Italic size={18} /></button>
